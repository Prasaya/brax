...
class DoubleHumanoid(env.Env):
    ...
    def __init__(self, **kwargs):
        # TODO: define a function to copy the system config automatically based on n_agents
        self.n_agents = 2
        # TODO: factorize environment properly
        humanoid1_config = text_format.Parse(_HUMANOID1_CONFIG, brax.Config())
        humanoid2_config = text_format.Parse(_HUMANOID2_CONFIG, brax.Config())
    ...
    def _get_obs(self, qp: brax.QP, 
        ...
        com_inert = self.inertia + self.mass.reshape(
            (11*2, 1, 1)) * ((jnp.linalg.norm(disp_vec, axis=1)**2.).reshape(
                (11*2, 1, 1)) * jnp.stack([jnp.eye(3)] * 11*2) - v_outer(disp_vec))
    ...
    def _get_qpos_qvel(self, qp: brax.QP) -> Tuple[jnp.ndarray, jnp.ndarray]:
        """
        ...
        qpos = [
            qp.pos[0, 2:], qp.rot[0], 
            joint_1d_angle[0], 
            joint_2d_angle[0], joint_2d_angle[1], 
            joint_3d_angle[0], joint_3d_angle[1], joint_3d_angle[2]
        ]
...
joints {
  name: "right_elbow2"
  stiffness: 15000.0
  parent: "right_upper_arm2"
  child: "right_lower_arm2"
  parent_offset {
    x: 0.18
    y: -0.18
    z: -0.18
...
joints {
  name: "left_elbow2"
  stiffness: 15000.0
  parent: "left_upper_arm2"
  child: "left_lower_arm2"
  parent_offset {
    x: 0.18
    y: 0.18
    z: -0.18
...
_SYSTEM_CONFIG = _HUMANOID1_CONFIG + _HUMANOID2_CONFIG + _ENV_CONFIG
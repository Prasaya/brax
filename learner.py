...
flags.DEFINE_integer("num_rlds_episodes", 0, "Number of episodes to output to RLDS.")
...
flags.DEFINE_enum("head_type", "", ["", "clip", "tanh"], "Which policy head to use.")
# ARS hps.
flags.DEFINE_integer(
...
def ppo_progress_fn(num_steps, metrics, params, make_policy, env, writer: metrics.Writer, ):
    print(
        "Environment steps : ",
        num_steps,
        "      Reward : ",
    ...
    writer.write_scalars(num_steps, metrics)
    ...
    with metrics.Writer(FLAGS.logdir) as writer:
        ...
        if FLAGS.learner == "ppo":
            make_policy, params, _ = ppo.train(
                ...
                progress_fn=functools.partial(ppo_progress_fn, writer=writer),
            )
